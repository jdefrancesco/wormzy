...
func main() {
	var (
		...
		filePath = flag.String("file", "", "path to send (send mode)")
		code     = flag.String("code", "", "pairing code")
		relay    = flag.String("relay", "127.0.0.1:9999", "rendezvous host:port")
		relayPin = flag.String("relay-pin", "", "base64(SHA256(SPKI)) to pin relay TLS; leave empty to skip pinning")
		stunServ = flag.String("stun", "stun.cloudflare.com:3478", "STUN server for public mapping")
		timeout  = flag.Duration("timeout", 60*time.Second, "NAT punch timeout")
	)
	flag.Parse()
	...
	if *code == "" {
		fatal("-code required (short human code)")
	}

	// 1) One UDP socket for STUN, punching, and QUIC
	...
	// 2) Rendezvous (TLS, optional SPKI pin)
	peer, assigned := rendezvousAndPAKE(*relay, *relayPin, *mode, *code, self)
	if *mode == "send" {
		fmt.Printf("[*] Share code: %s\n", assigned)
	}
	fmt.Printf("[*] Peer public: %s\n", peer.Public)

	peerUDP, err := net.ResolveUDPAddr("udp4", peer.Public)
	check(err)
	...
	var qconn *quic.Conn
	if *mode == "send" {
		listener, err := t.Listen(serverTLS, qconf)
		check(err)
		...
		qconn, err = t.Dial(contextWithDeadline(40*time.Second), peerUDP, clientTLS, qconf)
		check(err)
	...
	// 5) Noise XXpsk2 (initiator = dialer/recv)
	psk := pskCache.Load() // from PAKE
	fileKey, err := runNoiseOverQUIC(qconn, *mode == "recv", psk)
	check(err)

	// 6) Encrypting file transfer
	if *mode == "send" {
		check(sendFileEncrypted(qconn, *filePath, fileKey))
		fmt.Println("[*] transfer complete.")
	} else {
		receiveFilesEncryptedForever(qconn, fileKey)
		fmt.Println("[*] ready; Ctrl-C to exit.")
		select {}
	}
}

/*** Rendezvous + TLS + PAKE (SPAKE2) ***/

type tlsPinConfig struct {
	pin []byte // SHA256(SPKI)
}

func tlsPinnedConfig(pinB64 string) *tls.Config {
	var pin []byte
	if pinB64 != "" {
		dec, _ := base64.StdEncoding.DecodeString(pinB64)
		if len(dec) == 32 {
			pin = dec
		}
	}
	return &tls.Config{
		...
		InsecureSkipVerify: pin == nil, // dev mode if no pin
...
type pakeStore struct {
	mu  sync.Mutex
	psk []byte
}

func (p *pakeStore) Save(k []byte) { p.mu.Lock(); p.psk = append([]byte(nil), k...); p.mu.Unlock() }
func (p *pakeStore) Load() []byte {
	p.mu.Lock()
	defer p.mu.Unlock()
	return append([]byte(nil), p.psk...)
}

var pskCache pakeStore

func rendezvousAndPAKE(relayAddr, relayPin, mode, code string, me SelfInfo) (peer SelfInfo, outCode string) {
	// TLS (or plaintext if relay not using TLS; leave pin empty to allow)
	var conn net.Conn
	var err error
	if relayPin != "" {
		conn, err = tls.Dial("tcp", relayAddr, tlsPinnedConfig(relayPin))
	} else {
		// still try TLS first; if fails, fall back to plaintext
	...
	// Hello
	writeMsg(conn, "hello", map[string]string{"role": mode, "code": code})

	// Code (assigned/echoed)
	msg := mustReadMsg(r)
	if msg.Type != "code" {
	...
	// Send SelfInfo
	writeMsg(conn, "self", me)

	// === SPAKE2 over relay (derive PSK from human code) ===
	psk, err := runPAKEOverRelay(r, conn, mode, outCode, "send", "recv")
	check(err)
	pskCache.Save(psk)

	// After PAKE, relay will deliver peer info and close
	msg = mustReadMsg(r)
	if msg.Type != "peer" {
...
// SPAKE2 (using a simple two-message flow via relay)
// NOTE: Package API may vary slightly by version; adjust if needed.
func runPAKEOverRelay(r *bufio.Reader, w io.Writer, role, code, idA, idB string) ([]byte, error) {
	...
	if role == "send" {
		...
		if err := writeMsg(w, "pake1", msgA); err != nil { return nil, err }
		m := mustReadMsg(r)
		if m.Type != "pake1" { return nil, fmt.Errorf("want pake1, got %s", m.Type) }
		var msgB []byte
		_ = json.Unmarshal(m.Body, &msgB)
		keyA, err := st.Finish(msgB)
		if err != nil { return nil, err }
		_ = writeMsg(w, "pake2", []byte{}) // placeholder so relay can pair
		return keyA, nil
	}
	// responder
	m := mustReadMsg(r)
	...
	if err != nil { return nil, err }
	if err := writeMsg(w, "pake1", msgB); err != nil { return nil, err }
...
/*** Noise XXpsk2 over QUIC control stream → file key ***/
...
// runNoiseOverQUIC builds a transcript and HKDF-derives the file key
// from (PSK || transcript). Works with noise versions that lack XXpsk2.
func runNoiseOverQUIC(conn qConn, initiator bool, psk []byte) ([]byte, error) {
	...
	if initiator {
		...
		if err != nil { return nil, err }
		appendT(msg)
		if err := writeFrame(msg); err != nil { return nil, err }
		in, err := readFrame(); if err != nil { return nil, err }
		appendT(in)
		_, _, _, err = hs.ReadMessage(nil, in); if err != nil { return nil, err }
		msg, _, _, err = hs.WriteMessage(nil, nil); if err != nil { return nil, err }
		appendT(msg)
		if err := writeFrame(msg); err != nil { return nil, err }
	} else {
		in, err := readFrame(); if err != nil { return nil, err }
...
/*** Encrypted file transfer (XChaCha20-Poly1305 on uni stream) ***/
...
func sendFileEncrypted(conn *quic.Conn, path string, key []byte) error {
	...
	// Send base nonce (24 bytes) in clear
	...
	// Header: [u16 nameLen][u64 size] + name
...
func receiveFilesEncryptedForever(conn *quic.Conn, key []byte) {
	go func() {
		for {
...
func makeNonce(base [24]byte, ctr uint64) []byte {
	// XChaCha20-Poly1305 expects 24B nonce; we XOR in ctr on the tail.
	b := base
	for i := 0; i < 8; i++ {
		b[23-i] ^= byte(ctr >> (8 * i))
	}
...
/*** Utilities: filenames, TLS, STUN, ctx ***/
...
func stunDiscoverAny(conn *net.UDPConn, servers []string, rto time.Duration) (*net.UDPAddr, error) {
	...
	return nil, fmt.Errorf("no STUN response")
}
